...
void DBMVM(graph *og, dynamic_graph *dg, match_tree *mt, int times)
{
    ...
    // 按照轮次处理(两种轮次嵌套，行列嵌套)
    int round = 0;
    int *rnd = (int *)malloc(sizeof(int) * og->n);
    memset(rnd, -1, sizeof(int) * og->n);
                ...
                while (col_que_start != col_que_end)
                {
                    ...
                    do
                    {
                        for (start_ptr = dg->vtx_ptr_start[iter_vtx]; start_ptr < dg->vtx_ptr_end[iter_vtx]; start_ptr++)
                        {
                            ...
                            if (dg->degree[temp_row] == 0 || visited[2][temp_row] == v || rnd[temp_row] == round || temp_row == -1) // 是否要判断度数                             访问的每一个行顶点要判断轮次
                            {
                                continue;
                            }
                ...
                for (int i = 1; i < col_que_end; i++)
                {
                    ...
                    do
                    {
                        for (start_ptr = dg->vtx_ptr_start[iter_vtx]; start_ptr < dg->vtx_ptr_end[iter_vtx]; start_ptr++)
                        {
                            ...
                            {
                                rnd[dg->cur_edge[start_ptr]] = round;                  // 标记被访问过
                                ...
                                else // 已经被记录的行需要被移除，防止连接时记录了两条一样的边
                                {
                                    ...
                                    {
                                        bucket3.push(dg->cur_edge[start_ptr]);
                                    }
    ...
    // auto end_dbmvm = std::chrono::system_clock::now();
    // std::chrono::duration<double> elapsed_seconds_dbmvm = end_dbmvm - end_kasi;
